...
public final class FlippedMinHeapSort extends Sort {
    ...
    public void siftDown(int[] array, int length, int root, int dist) {
        while (root <= dist / 2) {
            int leaf = 2 * root;
            if (leaf < dist && Reads.compareIndices(array, length - leaf, length - leaf - 1, 0.5, true) == 1) {
                leaf++;
            }
            Highlights.markArray(1, length - root);
            Highlights.markArray(2, length - leaf);
            Delays.sleep(1);
            if (Reads.compareIndices(array, length - root, length - leaf, 0.5, true) == 1) {
