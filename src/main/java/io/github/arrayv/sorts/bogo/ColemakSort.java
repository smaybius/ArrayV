...
*/
public final class ColemakSort extends BogoSorting {

    int min;
    int max;

    public ColemakSort(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        this.setSortListName("Colemak");
        this.setRunAllSortsName("Colemak Sort");
        this.setRunSortName("Colemaksort");
    ...
    protected void handleItems(int[] array, int currentLength) {
        ...
        if (randBoolean()) {
            if (array[i] == min)
                Writes.write(array, i, max + 1, delay, true, false);
            Writes.write(array, i, array[i] - 1, delay, true, false);
        } else {
            if (array[j] == min)
                Writes.write(array, j, max + 1, delay, true, false);
