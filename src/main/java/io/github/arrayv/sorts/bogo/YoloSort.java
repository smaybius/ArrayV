...
public final class YoloSort extends BogoSorting {
    ...
    public YoloSort(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        this.setSortListName("Yolo");
        this.setRunAllSortsName("Yolo Sort");
        this.setRunSortName("Yolosort");
    ...
    public void runSort(int[] array, int currentLength, int bucketCount) {
        if (!isArraySorted(array, currentLength)) {
            ...
            while (!finalized) {
                for (int i = 0; i < currentLength; i++)
                    Writes.write(array, i, init[randInt(0, currentLength)], delay, true, false);
