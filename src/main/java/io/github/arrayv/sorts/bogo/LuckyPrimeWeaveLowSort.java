...
final public class LuckyPrimeWeaveLowSort extends BogoSorting {
    public LuckyPrimeWeaveLowSort(ArrayVisualizer arrayVisualizer) {
        ...
        this.setRunAllSortsName("Lucky Prime Weave Sort (Low Prime)");
        this.setRunSortName("Lucky Prime Weave (Low Prime)");
        ...
        while (!testpass) {
            ...
            if (gap == 1) {
                ...
                while (boundi < currentLength && testpass) {
                    if (Reads.compareIndices(array, boundi - 1, boundi, 0.125, true) <= 0)
                        boundi++;
                    else {
                        testpass = false;
                        lastbound = boundi;
