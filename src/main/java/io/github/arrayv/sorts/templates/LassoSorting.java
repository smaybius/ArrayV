...
public abstract class LassoSorting extends Sort {
    ...
    protected void NQSort(int[] array, int a, int b, int c, int d) {
        if (d - a == 2) {
            ...
            if (Reads.compareIndices(array, a, d, 0.5, true) == 1) {
        ...
        if (Reads.compareIndices(array, b, d, 0.5, true) == 1) {
            t1 = array[d];
            Writes.write(array, d, array[b], 1, true, false);
        }

        if (Reads.compareIndices(array, a, c, 0.5, true) == 1) {
            t2 = array[c];
            Writes.write(array, c, array[a], 1, true, false);
        }

        if (Reads.compareIndices(array, c, d, 0.5, true) == 1) {
        ...
        if (Reads.compareIndices(array, b, c, 0.5, true) == 1) {
    ...
    protected void mergeRight(int[] array, int start, int mid, int end) {
        ...
        while (left >= start && right >= BufLoc) {
            if (Reads.compareIndices(array, left, right, 0.5, true) == 1) {
    ...
    protected boolean mergeIndex(int[] array, int start, int mid, int end) {
        ...
        for (int i = 1; i < r; i++) {
            ...
            if (lastWasHi != hi || f < BufSize || Reads.compareIndices(array, n - 1, n, 0.5, true) == 1)
