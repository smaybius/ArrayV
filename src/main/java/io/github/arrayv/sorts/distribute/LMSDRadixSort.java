...
final public class LMSDRadixSort extends Sort {
    ...
    public ArrayVList[] radixSort(int[] array, int start, int end, int place) {
        ArrayVList[] registers = new ArrayVList[this.base];
    ...
    public void lmsdRadixSort(int[] array, int mina, int maxa, int place, int maxPlace) {
        if (maxPlace < place || maxa - mina <= 1)
            return;

        ArrayVList[] registers = this.radixSort(array, mina, maxa, place);
