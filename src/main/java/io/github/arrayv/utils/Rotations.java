...
public final class Rotations {
    ...
    // utility functions
    // utility functions
    private static void swapBlocksBackwards(int[] array, int a, int b, int len, double pause, boolean mark,
            boolean auxwrite) {
        for (int i = 0; i < len; i++) {
    ...
    private static void shiftForwards(int[] array, int start, int length, double pause, boolean mark,
            boolean auxwrite) {
    ...
    private static void shiftBackwards(int[] array, int start, int length, double pause, boolean mark,
            boolean auxwrite) {
    ...
    // rotation algorithms
    public static void griesMills(int[] array, int pos, int lenA, int lenB, double pause, boolean mark,
            boolean auxwrite) {
        while (lenA != 0 && lenB != 0) {
            if (lenA <= lenB) {
    ...
    public static void threeReversal(int[] array, int pos, int lenA, int lenB, double pause, boolean mark,
            boolean auxwrite) {
    ...
    public static void holyGriesMills(int[] array, int pos, int lenA, int lenB, double pause, boolean mark,
            boolean auxwrite) {
        while (lenA > 1 && lenB > 1) {
            while (lenA <= lenB) {
                blockSwap(array, pos, pos + lenA, lenA, pause, mark, auxwrite);
    ...
    public static void cycleReverse(int[] array, int pos, int lenA, int lenB, double pause, boolean mark,
            boolean auxwrite) {
        if (lenA < 1 || lenB < 1)
            return;
    ...
    public static void juggling(int[] array, int pos, int lenA, int lenB, double pause, boolean mark,
            boolean auxwrite) {
        ...
        for (int cnt = 0,
                ...
                startIndex = index; cnt < length; cnt++) {
    ...
    public static void radon(int[] array, int pos, int lenA, int lenB, double pause, boolean mark, boolean auxwrite) {
        ...
        if (min < 1)
            return;
        if (bridge <= min)
            Writes.reversal(array, pos + min, pos + max - 1, pause, mark, auxwrite);
        for (int i = 0; i < min; i++)
            Writes.swap(array, pos + i, pos + max + i, pause, mark, auxwrite);
        if (bridge == 0)
            return;
    ...
    public static void centered(int[] array, int pos, int lenA, int lenB, double pause, boolean mark,
            boolean auxwrite) {
        if (lenA < 1 || lenB < 1)
            return;
        if (lenA < lenB) {
    ...
    public static void adaptable(int[] array, int pos, int lenA, int lenB, double pause, boolean mark,
            boolean auxwrite) {
        if (lenA == 0 || lenB == 0)
            return;
        if (lenA % lenB == 0 || lenB % lenA == 0)
            Rotations.holyGriesMills(array, pos, lenA, lenB, pause, mark, auxwrite);
        else
            Rotations.cycleReverse(array, pos, lenA, lenB, pause, mark, auxwrite);
    }
}
